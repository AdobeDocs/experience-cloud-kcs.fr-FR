...
Puisqu’il s’agit d’un sujet plus important, reportez-vous à la section [cet article](https://helpx.adobe.com/fr/experience-manager/kb/optimizing-the-dispatcher-cache.html) pour plus d’informations sur l’optimisation de la variable [!DNL dispatcher] cache.
...
1. Si le site comporte du contenu personnalisé/dynamique :
   ...
   1. Utilisation [[!DNL Apache] Inclusions dynamiques Sling](https://helpx.adobe.com/experience-manager/kt/platform-repository/using/sling-dynamic-include-technical-video-setup.html) pour séparer le contenu afin que différentes parties de la page puissent être mises en cache pendant différentes périodes.
