...
Activé [!DNL Linux] plateforme, `lsof` peut être utilisée pour déboguer les ressources qui sont conservées ouvertes par le processus.
...
<br><br><br><br><br> <br><br>
Inspect : la sortie de la `lsof-sorted-counts-\*.txt` fichier .  Il indique les fichiers ou les sockets qui sont actuellement conservés ouverts par le processus.
...
Le code personnalisé qui crée un service Web est une cause courante de la persistance des sockets ouverts.  Dans de nombreux cas, des bibliothèques telles que [!DNL Apache] [!DNL Commons HttpClient] est utilisée, mais les connexions ne sont jamais fermées par les développeurs.  Voir [cet article](https://stackoverflow.com/questions/43454514/proper-usage-of-apache-httpclient-and-when-to-close-it) pour plus d’informations sur [!DNL Apache] [!DNL Commons HttpClient].
...
2. Ajouter la variable `CQ_MAX_OPEN_FILES` en haut du script :    `CQ_MAX_OPEN_FILES=8192 export CQ_MAX_OPEN_FILES`


Si l’erreur s’affiche `-bash: ulimit: open files: cannot modify limit: Operation not permitted` lors du démarrage d’AEM, la configuration ci-dessus ne fonctionne pas.
...
Parfois, la limite utilisateur est suffisamment élevée, mais le système lui-même a atteint son nombre maximum de fichiers. Exécutez les opérations suivantes comme `su/root` user :
...
2. Ajoutez cette ligne à `/etc/sysctl.conf` pour augmenter la valeur maximale du fichier d’ouverture du système :    `fs.file-max = 300000`
