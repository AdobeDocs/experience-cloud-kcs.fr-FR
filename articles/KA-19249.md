...
Vous pouvez afficher les références locales d’une ressource sur [!UICONTROL Références] de [!UICONTROL Propriétés]. (Dans [!DNL Assets] Console, sélectionnez une ressource, puis sélectionnez [!UICONTROL Propriétés] dans la barre d’outils, puis sélectionnez [!UICONTROL Références] ). Lorsque vous utilisez les informations en dehors d’AEM, vous pouvez les exporter au format JSON.
...
L’accès à l’URL suivante 1 renvoie les références locales au format JSON. `path` les valeurs de la réponse sont les chemins de contenu référençant une ressource cible.
