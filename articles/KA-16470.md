...
Ce guide d’intégration technique est destiné aux consultants en Adobe, aux partenaires d’Adobe et à d’autres spécialistes expérimentés de l’implémentation.
...
4. La gestion dynamique des balises lit la variable `CRM_ID`
La configuration se trouve dans <b>service de Marketing Cloud ID</b> outil.

Le `hashed_email` L’élément de données est configuré dans la gestion dynamique des balises en lisant la variable `CRM_ID` disponible dans dataLayer.
...
Deux sources de données doivent être créées : l’un en fonction de l’identifiant de Marketing Cloud, l’autre en fonction de l’identifiant Declared ID.
...
Deux sources de données seront créées dans l’interface utilisateur d’Adobe Campaign : l’un en fonction de l’ID déclaré ou de l’ID client haché, l’autre en fonction de l’UUID du demandeur ou de l’identifiant Marketing Cloud.

L’identifiant de destination correspondant à chaque identifiant est ajouté dans les champs respectifs.

Par exemple : l’ID de destination pour AAM destination Adobe Campaign - Declared ID est ajouté dans le paramètre de source de données avec la clé de réconciliation comme ID de client haché.
...
Sous <b>Administration</b> dans l’interface utilisateur de la campagne, sélectionnez <b>Production</b><b>Workflows techniques</b>.
...
Une fois créés, ces segments apparaissent automatiquement dans l’interface utilisateur d’AAM, sous <b>Caractéristiques</b><b>Toutes les caractéristiques</b><b>Caractéristiques Adobe Campaign</b>.
