---
title: "Audience Manager à Campaign"
...
2. La synchronisation de la base de données CRM avec le client Campaign doit configurer un processus dans lequel il envoie ses données CRM vers Campaign. L’e-mail, CRM_ID, sont les champs minimum à synchroniser.
...
4. La gestion dynamique des balises lit que la configuration de l’ID de gestion de la relation client se trouve dans l’outil &quot;service de Marketing Cloud ID&quot;. L’élément de données hashed_email est configuré dans la gestion dynamique des balises en lisant le CRM_ID disponible dans dataLayer.
...
Le sFTP créé pour AAM doit être configuré avec Adobe Campaign afin que Campaign puisse pousser et extraire des segments d’AAM. Les informations d’identification sFTP vous seront fournies par l’assistance clientèle Adobe ou votre représentant d’Adobe. Lors de l’approvisionnement, les emplacements ftp-in et ftp-out sont configurés. Ils sont utilisés par campaign pour envoyer des audiences à AAM et des segments intégrés d’AAM vers campaign respectivement.
...
6. &#x200B; &#x200B;  de  de données &#x200B; créer des sources de données et des destinations avec
...
7. &#x200B; &#x200B; Adobe Campaign de  de données &#x200B; Configurez les sources de données dans Les sources de données sont ensuite configurées dans Adobe Campaign. Deux sources de données seront créées dans l’interface utilisateur d’Adobe Campaign. l’un basé sur un ID déclaré ou un ID de client haché, l’autre basé sur l’UUID du demandeur ou de marketing cloud. L’identifiant de destination correspondant à chaque identifiant est ajouté dans les champs respectifs. Par exemple, l’ID de destination pour AAM destination Adobe Campaign - Declared ID est ajouté au paramètre de la source de données avec la clé de réconciliation comme ID de client haché.
...
8. Suivi du comportement des utilisateurs sur le site Dans le cas d’utilisation ci-dessous, le mot-clé de recherche utilisé par un client est capturé dans evar2 et un achat de produit en tant qu’événement d’achat dans Analytics. Le flux ci-dessous est basé sur le transfert côté serveur des appels AA vers AAM. Sur la page de recherche, nous capturons le paramètre &quot;q&quot; et le mettons dans un élément de données :
...
Ensuite, nous capturons également l’événement &quot;purchase&quot; dans analytics sur la page de remerciement :
...
À ce stade, nous pouvons suivre nos clients et créer des segments dans l’Audience Manager. En outre, nous avons assuré que Audience Manager et Campaign collectent les utilisateurs avec le même CRM_ID.

9. &#x200B; &#x200B;  de  de  &#x200B; Créez des segments dans en fonction du comportement de l’utilisateur. Créez ensuite les caractéristiques suivantes : &quot;A cherché l&#39;hiver&quot; :
...
10. &#x200B; &#x200B; Adobe Campaign de  de &#x200B; Poussez le segment vers Campaign Pour partager une audience AMC avec Campaign, le paramétrage doit être effectué dans une fois le segment créé dans. Commencez par créer une audience partagée dans Adobe Campaign et sélectionnez le segment AMC dans la liste de segments. Deux audiences partagées doivent être créées en fonction du MID et de l’ID déclaré dans la campagne. Une fois la configuration terminée, les segments peuvent être vus comme mappés automatiquement aux destinations respectives dans AAM.
...
11. Partager une audience de campagne avec AAM via AMC Pour partager une audience de campagne avec AAM via AMC, la configuration suivante est requise dans Adobe Campaign. Sous l’option de menu Administration de l’interface utilisateur de Campaign, sélectionnez les workflows techniques de production. Cliquez sur les nouveaux workflows techniques, faites glisser la requête, mettez à jour l&#39;audience partagée, puis terminez.
...
Dans la campagne, créez d’abord une liste basée sur une audience. Elle se trouve dans les &quot;listes&quot; :
...
La fenêtre contextuelle vous permet désormais de sélectionner une audience Marketing Cloud. Sélectionnez &quot;search_for_winter_did_not-buy&quot; :
...
Il se peut que vous ne vous soyez pas connecté à l’aide de votre Adobe ID pour Campaign. Si vous vous êtes connecté à l’aide de votre Adobe ID, assurez-vous d’avoir été ajouté au produit Bibliothèque d’audiences dans l’interface utilisateur de Marketing Cloud Enterprise Console.
...
3. Ne jamais mapper les segments à la destination de la campagne à partir de l’interface utilisateur AAM &#x200B;
