...
# AEM problèmes de configuration GraphQl
...
Le client développe un site web qui doit récupérer les données de ses instances AEM à l’aide d’AEM solution sans interface utilisateur.

Pour configurer le point de terminaison, suivez le tutoriel de la rubrique `[` 1`]`  a été suivie.

Après avoir publié tous les fragments de contenu, tous les modèles, contenus, références, ressources et requêtes, comme décrit dans la section `[` 2`]` , lors de la tentative de récupération d’un fragment de contenu à partir de l’instance de publication, une erreur 404 est reçue empêchant la lecture du contenu par l’éditeur.
...
`[` 1`]`  aem_base_version : 6.5 aem_envId : xxxxxxxxx aem_envType : dev aem_program_id : xxxxxxxx aem_release_id : xxxxx aem_service : cm-pxxxxxx-exxxxxxx aem_tenant : ns-équipe-aem-prd-nxxxxxxxx_server: publish backend:/- cache : <b>bloqué</b>
grappe : `<` grappe`>`
code : - file_path : /var/log/aem/dispatcher.log hôte : publish-pxxxxxx-exxxxxxx.adobeaemcloud.com méthode : espace de noms du POST : ns-Team-aem-cm-prd-nxxxxxx orig_time: 25/Jul/2023:15:39:11 +0000 path: /content/cq:graphql/graphql/endpoint.json pod_name: `<` pod_name`>`
pod_uid : `<` pod_uid`>`
