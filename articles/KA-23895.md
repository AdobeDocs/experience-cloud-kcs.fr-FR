---
title: '« Adobe Experience Manager : comment résoudre BUILD_NPM_AUDIT_ERROR dans le pipeline front-end as a Cloud Service d’AEM »'
...
keywords: « KCS, dépannage, Adobe Experience Manager as a Cloud Service, AEM as a Cloud Service, BUILD_NPM_AUDIT_ERROR, AEMaaCS, pipeline front-end »
...
# Adobe Experience Manager : résolution de BUILD_NPM_AUDIT_ERROR dans le pipeline front-end as a Cloud Service d’AEM


Supprimez deux nœuds dans votre code de projet AEM, comme décrit dans la documentation de cet article.
...
Le fichier build.log contient un BUILD_NPM_AUDIT_ERROR.
...
Supprimez les deux nœuds suivants dans votre code de projet AEM, comme décrit dans la documentation`[` 1`]` :

1. /conf/`<` site-name`>` /sling:configs/com.adobe.aem.wcm.site.manager.config.SiteConfig
2. /conf/`<` site-name`>` /sling:configs/com.adobe.cq.wcm.core.components.config.HtmlPageItemsConfig

Le pipeline front-end met à jour le nœud SiteConfig avec le nom du package du thème déployé, mais il échoue si le nom du package et les paramètres SiteConfig sont différents.
...
Étape 2. Supprimez les deux définitions de nœud :
...
Suite à l’étape 3, AEM recrée correctement ces nœuds de configuration. Le pipeline doit réussir par la suite.
