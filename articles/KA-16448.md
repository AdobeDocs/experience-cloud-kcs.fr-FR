...
1. Copiez le chemin d’accès des noeuds utilisateur anonymes et admin à partir des résultats de l’étape 1 (<b>Étape 1 : Recherche des utilisateurs administrateurs et anonymes</b>) ci-dessus.
...
17. Répétez les étapes 8 à 16 pour tous les autres chemins d’accès en modifiant la commande curl du chemin d’accès. Par exemple, les listes de contrôle d’accès situées sous `/etc` au lieu de `/content`: 
