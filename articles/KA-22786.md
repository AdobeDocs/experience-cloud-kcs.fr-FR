---
title: Impossible de charger la console Ressources DAM dans l’auteur
description: Découvrez comment résoudre le problème AEM où vous ne pouvez pas charger la console Ressources de gestion des actifs numériques dans l’instance d’auteur. Nettoyez les instances de workflow.
solution: Experience Manager
product: Experience Manager
applies-to: Experience Manager
keywords: "KCS, impossible de charger la console Ressources de gestion des actifs numériques, auteur, AEM, workflow"
...
Si un écran vierge s’affiche lors de l’accès à la console d’AEM des ressources dans l’auteur, cela peut être dû au trop grand nombre d’instances de workflow qui entraînent une erreur. `[` 1`]` . Vous pouvez le résoudre en nettoyant ces instances.
...
Lors de la tentative d’accès à la console AEM ressources à l’aide de &quot;`/assets.html/content/dam`&quot; l’écran est vide.
...
Lors de la vérification du journal des erreurs, erreur `[` 1`]`  a été trouvé.

Les interfaces utilisateur Assets et Sites doivent rechercher l’état du workflow des éléments qu’elles répertorient. Par conséquent, s’il y a trop d’instances de workflow, la page ne se charge pas et échoue. `[` 1`]`  sera présent dans les journaux.
