---
title: "Configuration de l’Adobe I/O : Authentification - étape par étape"
description: Description
solution: Experience Cloud
product: Experience Cloud
applies-to: "Experience Cloud"
keywords: "KCS"
resolution: Resolution
internal-notes: null
bug: false
article-created-by: Roxann McGlumphy
article-created-date: "10/25/2021 7:27:05 PM"
article-published-by: Roxann McGlumphy
article-published-date: "10/25/2021 7:41:01 PM"
version-number: 1
article-number: KA-17522
dynamics-url: "https://adobe-ent.crm.dynamics.com/main.aspx?forceUCI=1&pagetype=entityrecord&etn=knowledgearticle&id=c08d1f85-c935-ec11-b6e6-000d3a3485ea"
exl-id: 6f4f5c14-e40c-43db-b867-a250e01a9a05
source-git-commit: 0c3e421beca46d9fe1952b1f98538a50697216a0
workflow-type: tm+mt
source-wordcount: '1448'
ht-degree: 0%

---

# Configuration de l’Adobe I/O : Authentification - Étape par étape

## Description

<br>Ce document vous explique comment créer les certificats, clés et jetons à exécuter.<br><br><br><br><br>Objectif<br><br><br><br> <br><br>
Pour fournir des étapes claires et reproductibles afin de générer un jeton porteur d’Adobe I/O pour exécuter des appels API pour des solutions DMA telles que [!DNL Target].


## Résolution

<br><br>Étapes<br><br><br><br> <br><br>
<b>Procédure de création d’un jeton porteur pour exécuter les appels API d’Adobe I/O :</b>

1. Génération d’une clé privée et d’un certificat public
2. Création d’une intégration dans la console Adobe I/O
3. Générer un jeton JWT
4. Échanger le jeton JWT pour un jeton porteur d’accès
5. Jeton du porteur d’accès utilisateur pour exécuter les appels API


- <b>Générez une clé privée et un certificat public selon la documentation suivante :</b> https://www.adobe.io/apis/cloudplatform/console/authentication/createcert.html


   Sur Mac, les commandes suivantes sont intégrées à un terminal. 

   Sur un ordinateur, vous devez télécharger [!DNL Cygwin] (ou tout autre outil de préférences personnelles) et exécutez-le à partir de la ligne de commande. Les étapes à télécharger et à installer sont les suivantes : [!DNL Cygwin]:

   1. Accédez à https://cygwin.com/install.html
   2. Téléchargement et exécution `setup-x86_64.exe`

   <b>*Remarque</b>:  Votre répertoire personnel est le suivant : `C:\cygwin64\home\WINDOWSUSER`

   Vous pouvez rechercher et installer des packages supplémentaires pendant la phase d’installation. Je vous recommande d&#39;installer tout ce qui concerne &quot;`curl`&quot; et &quot;`ssh.`&quot;*


   Voici la commande à exécuter :
   <br> <br> <br>

   | `$ openssl req -nodes -text -x509 -newkey rsa:2048 -keyout secret.pem -out certificate.pem -days 356` |
   | --- |

<br> 

    &lt;b>Remarque&lt;/b> :  Il vous pose plusieurs questions à remplir pour générer le certificat, voir la capture d’écran ci-dessous :
![rtaImage](https://helpx.adobe.com/content/dam/help/en/marketing-cloud-core/kb/adobe-io-authentication-step-by-step/jcr%3acontent/main-pars/procedure/proc_par/step_0/step_par/image/rtaImage.png "rtaImage")<br> 

    Une fois les fichiers créés, vous convertissez le `secret.key` en `secret.pem` à l’aide de la commande suivante :
<br> <br> <br>

    | `$ openssl pkcs8 -topk8 -informez PEM -outform DER -` `in` `secret.pem -nocrypt secret.key` |
    | — |

<br> 

    Voici les fichiers créés sur votre système de fichiers (vous pouvez déplacer ces fichiers vers un autre emplacement de votre ordinateur à ce stade pour l’organisation) :
![rtaimage_1_](https://helpx.adobe.com/content/dam/help/en/marketing-cloud-core/kb/adobe-io-authentication-step-by-step/jcr%3acontent/main-pars/procedure/proc_par/step_0/step_par/image_747173003/rtaimage_1_.png "rtaimage_1_")

    Remarque :
    
    
    Les fichiers sont générés dans votre répertoire personnel : `C:\cygwin64\home\WINDOWSUSER`


- <b>Créez une intégration dans la console Adobe I/O :</b>

   - Accédez à [https://console.adobe.io/](https://console.adobe.io/)
   - Sélectionnez la société.
   - Cliquez sur le bouton &quot;Nouvelle intégration&quot;.

![rtaimage_2_](https://helpx.adobe.com/content/dam/help/en/marketing-cloud-core/kb/adobe-io-authentication-step-by-step/jcr%3acontent/main-pars/procedure/proc_par/step_1/step_par/image/rtaimage_2_.png "rtaimage_2_") 
- Sélectionnez l’option &quot;Accéder à une API&quot;, puis cliquez sur &quot;Continuer&quot;.

![rtaimage_3_](https://helpx.adobe.com/content/dam/help/en/marketing-cloud-core/kb/adobe-io-authentication-step-by-step/jcr%3acontent/main-pars/procedure/proc_par/step_1/step_par/image_374011898/rtaimage_3_.png "rtaimage_3_") 
- Sélectionnez Solution d’Adobe (actuellement disponible uniquement pour [!DNL Target]).

    &lt;b>Remarque :&lt;/b> L’utilisateur doit être un utilisateur de l’Experience Cloud ET avoir accès à cette solution.
![rtaimage_4_](https://helpx.adobe.com/content/dam/help/en/marketing-cloud-core/kb/adobe-io-authentication-step-by-step/jcr%3acontent/main-pars/procedure/proc_par/step_1/step_par/image_1407429561/rtaimage_4_.png "rtaimage_4_") 
- Sélectionnez &quot;Nouvelle intégration&quot; et cliquez sur &quot;Continuer&quot;.

![rtaimage_5_](https://helpx.adobe.com/content/dam/help/en/marketing-cloud-core/kb/adobe-io-authentication-step-by-step/jcr%3acontent/main-pars/procedure/proc_par/step_1/step_par/image_1811594053/rtaimage_5_.png "rtaimage_5_") 
- Remplissez le formulaire d’intégration.

![rtaimage_6_](https://helpx.adobe.com/content/dam/help/en/marketing-cloud-core/kb/adobe-io-authentication-step-by-step/jcr%3acontent/main-pars/procedure/proc_par/step_1/step_par/image_559705621/rtaimage_6_.png "rtaimage_6_") 
- Faites glisser &quot;certificate.pem&quot; du système de fichiers vers le formulaire à charger.

![rtaimage_7_](https://helpx.adobe.com/content/dam/help/en/marketing-cloud-core/kb/adobe-io-authentication-step-by-step/jcr%3acontent/main-pars/procedure/proc_par/step_1/step_par/image_1707339822/rtaimage_7_.png "rtaimage_7_") 
- Une fois le fichier téléchargé, cliquez sur le bouton &quot;Créer l’intégration&quot; - voir la capture d’écran : fileuploaded_createint.

![rtaimage_8_](https://helpx.adobe.com/content/dam/help/en/marketing-cloud-core/kb/adobe-io-authentication-step-by-step/jcr%3acontent/main-pars/procedure/proc_par/step_1/step_par/image_282868645/rtaimage_8_.png "rtaimage_8_") 
- Une fois le traitement terminé, cliquez sur &quot;Continuer vers les détails de l’intégration&quot;.

![rtaimage_9_](https://helpx.adobe.com/content/dam/help/en/marketing-cloud-core/kb/adobe-io-authentication-step-by-step/jcr%3acontent/main-pars/procedure/proc_par/step_1/step_par/image_1642793077/rtaimage_9_.png "rtaimage_9_")<br>     Succès ! Vous avez maintenant créé une intégration.
![rtaimage_10_](https://helpx.adobe.com/content/dam/help/en/marketing-cloud-core/kb/adobe-io-authentication-step-by-step/jcr%3acontent/main-pars/procedure/proc_par/step_1/step_par/image_378374931/rtaimage_10_.png "rtaimage_10_")


- <b>Générer un jeton Web JSON (JWT) :</b>
Dans l’interface utilisateur d’intégration, cliquez sur l’onglet JWT, collez dans la clé privée, cliquez sur le bouton Générer JWT - voir la capture d’écran : génération de JWT - entrée.

   ![rtaimage_11_](https://helpx.adobe.com/content/dam/help/en/marketing-cloud-core/kb/adobe-io-authentication-step-by-step/jcr%3acontent/main-pars/procedure/proc_par/step_2/step_par/image/rtaimage_11_.png "rtaimage_11_") 
   - Une fois généré, vous voyez le JWT et un exemple de commande CURL.
   - Cliquez sur l’icône &quot;Copier&quot; sous &quot;JWT généré&quot;.

![rtaimage_12_](https://helpx.adobe.com/content/dam/help/en/marketing-cloud-core/kb/adobe-io-authentication-step-by-step/jcr%3acontent/main-pars/procedure/proc_par/step_2/step_par/image_861805817/rtaimage_12_.png "rtaimage_12_")


- <b>Exchange JWT pour le jeton d’accès au porteur :</b>


   *Remarque : Si vous exécutez ceci sur un terminal Mac, la réponse semble être tronquée. À la place, utilisez [!DNL Postman].*

   - Téléchargez et installez l’outil d’API gratuit nommé &quot;[!DNL Postman]&quot; (disponible sur Mac, [!DNL Windows]ou [!DNL Linux]) : [https://www.getpostman.com/](https://www.getpostman.com/)
   - Importez cet appel d’api dans [!DNL Postman] en copiant le code ci-dessous dans un fichier texte sur votre ordinateur avec une `.json` extension :

<br> <br> <br>

    | `{` `&quot;id&quot;` `:` `&quot;f6854718-2800-64a8-238e-e785e344f6cf&quot;` `,` `&quot;name&quot;` `:` &quot;Exchange JWT for Bearer token&quot;`,` `&quot;description&quot;`:`&quot;&quot;`,` &quot;order&quot;` `:` &quot;048b6fc7-f1db-5028-ff21-45778613 e2c5&quot; `,` ` &quot;folder&quot;` `:,` ` &quot;folder_order&quot;` `:,` ` &quot;timestamp&quot;` `:1516812553075,` `&quot;owner&quot;` `:` ` &quot;860614&quot; `,` &quot;public&quot;` `:` ,` `&quot;requests&quot;` `:{` `&quot;id&quot;` `:` `&quot;048b6fc7-f1db-5028-ff21-45778613e2c5&quot;`,`&quot;name&quot;`:` &quot;Exchange JWT for Bearer token&quot;`,` `&quot;collectionId&quot;` `:` `,` `&quot;method&quot;` `:` ` &quot;POST&quot;` `,` ` &quot;description&quot;` `:` ` &quot;JWT exchange flow&quot;` `, ` &quot;headers&quot;` `&quot; `:`&quot;`&quot; `,` `&quot;dataMode&quot;` `:`&quot; ` &quot;params&quot;`,`&quot; `&quot;data&quot;` `:` &quot;value&quot; `:` `&quot;0fa5e762277c414f903649dd51424ac6&quot;`, `&quot;type&quot;` `:` `&quot;text&quot;` `},{` `&quot;key&quot;`:` &quot;client_secret&quot;`, `&quot;value&quot;` `:` `&quot;9ff026f2-dfa4-4228-8dfa-11d809d4706b`` `&quot;type&quot;`: `&quot;text&quot;` `},{` `&quot;key&quot;` `:` `&quot;jwt_token&quot;`,` `&quot;value&quot;`:`&quot;eyJ0eXAiJKV1QiLCJhbGciOiJSUzI1NiJ9.eyle HAiOjE0ODg4ODMzMzIsImlzcyI6IjY1NzhBNTU0NTZFODRFMjQ3RjAwMDEwMUBBZG9iZU9yZyIsInN1YiI6IjlDQj EyOTlENThCM0VDNkYwQTQ5NUM3RkB0ZWNoYWNjdC5hZG9iZS5jb20iLCJhdWQiJodHRwczovL2ltcy1uYTE YWRvYmVsb2dpbi5jb20vYy8wZmE1ZTc2MjI3N2M0MTRmOTAzNjQ5ZGQ1MTyNGFjNiImh0dHBzOi8W zLW5hMS5hZG9iZWxvZ2LmNvbS9zL2VudF9zbWFydGNvbnRlbnRfc2RrIjp0cnVlfQ.LwiDDjwUfZ2Z 89vfWd2ZVnfG-FwpQplKvzEecTLua_hvGNfQAZBPTHbVICPkeNjr41cRUr_OmNuOmtFOwVokUjd5r GOqBNWWKPAyAdXhBdE05oFa2Gar6adytKv-vf7gAnVQbv-PUADbCtmxoOygbafXi9V3ZHz1FBwPJ8vpnZH4ZH3zVf420XwnzLa9IB02nUciG_fQ0b1Qgj429Yi7m-lhW—2bMZKyNdSnioNaICFg6ASY1vnNm1zICPla24222 K_Lwzbrye8itgQStRUp1mH53Ww36xzqVxNIYLQCEoI9qxAJl0HQhaXeSPrU9PmcExIyKBim2CZzg&quot; `, `&quot;type &quot;` `:` `&quot;text&quot;` `},` `&quot;raw&quot;ModeData&quot;` `:`&quot; `&quot;`,` `&quot;url&quot;` `:` `&quot;https:\/\/ims-na1.adobelogin.com\/ims\/exchange\/jwt\/&quot;`, `&quot;responses&quot; `:,` `&quot;pathVariableData&quot;` `:` `:,` `&quot;headerData&quot;`:,` `&quot;auth&quot;` `:null,` `&quot;collection_id&quot;`:` `&quot;08283cc0-461e-155c-e07f-ca64bae1dcae&quot;`, `&quot;isFromCollection&quot; `` : true`,` `&quot;collectionRequestId&quot;` `:`:` &quot;29f7fc5f-7e6d-01d4-de86-2f273b8a6429&quot;`, `&quot;currentHelper&quot;` `:null,` `&quot;helperAttributes&quot;`:null}` |
    | — |

<br> 

    Exemple de nom de fichier : `exchangeJwt.json`
![rtaimage_13_](https://helpx.adobe.com/content/dam/help/en/marketing-cloud-core/kb/adobe-io-authentication-step-by-step/jcr%3acontent/main-pars/procedure/proc_par/step_3/step_par/image/rtaimage_13_.png "rtaimage_13_") 
- Importer le fichier dans [!DNL Postman] en sélectionnant le fichier Importer : vous pouvez faire glisser et déposer le `.json` ici ou recherchez le fichier.

![rtaimage_14_](https://helpx.adobe.com/content/dam/help/en/marketing-cloud-core/kb/adobe-io-authentication-step-by-step/jcr%3acontent/main-pars/procedure/proc_par/step_3/step_par/image_635611899/rtaimage_14_.png "rtaimage_14_") 
- Cela crée une &quot;collection&quot; dans [!DNL Postman] sur le côté gauche, avec un appel API intitulé &quot;Exchange JWT for Bearer token&quot;.

![rtaimage_15_](https://helpx.adobe.com/content/dam/help/en/marketing-cloud-core/kb/adobe-io-authentication-step-by-step/jcr%3acontent/main-pars/procedure/proc_par/step_3/step_par/image_755147621/rtaimage_15_.png "rtaimage_15_") 
- Cliquez sur l’appel API &quot;Exchange JWT for Bearer token&quot;.
- Dans la section principale de l’interface utilisateur, voici ce que vous voyez :

![rtaimage_16_](https://helpx.adobe.com/content/dam/help/en/marketing-cloud-core/kb/adobe-io-authentication-step-by-step/jcr%3acontent/main-pars/procedure/proc_par/step_3/step_par/image_287339656/rtaimage_16_.png "rtaimage_16_")<br> 

    Remarque : comme indiqué en rouge ci-dessus, je suis dans l&#39;onglet &quot;Corps&quot;. Si vous vous trouvez dans un autre onglet, vous ne voyez pas les trois informations requises.
    
    
    
    Informations nécessaires :
    
    &lt;b>client_id&lt;/b> : obtenir de la page d’aperçu de l’intégration
    
    &lt;b>client_secret&lt;/b> : générer sur la page d’aperçu de l’intégration ;
    
    &lt;b>jwt_token&lt;/b>: coller dans le jeton JWT généré que vous avez copié à l’étape précédente
    
    
    
    Cliquez sur &quot;Envoyer&quot;.
    
    Le jeton porteur est récupéré, qui est utilisé pour effectuer des appels API.
![rtaimage_17_](https://helpx.adobe.com/content/dam/help/en/marketing-cloud-core/kb/adobe-io-authentication-step-by-step/jcr%3acontent/main-pars/procedure/proc_par/step_3/step_par/image_1624742308/rtaimage_17_.png "rtaimage_17_")<br> 

    access_token est utilisé dans le cadre des appels d’API que vous prévoyez d’exécuter.
<br> <br> <br>

    | `&quot;access_token&quot;`: `&quot;eyJ4NXUiOiJpbXNfbmExLWtleS0xLmNlciIsImFsZyI6IlJTMjU2In0.eyJpZCI6IjE1MTY3gND xNjU5MzZfMjdiNTkwYmUtYjVlYy00ZjhiLTkzNWEtZTAyMjZmYTZiYTk1X3VlMSIsImNsaWVud9pZCI6IjQyOTkxMzRlNzRkNTRkNTZhY2YyYTc4YjcyYTdlNDFiwidXNlcl9pZCI6IjkwNTEyQTlUE2L N0I3ODEwQTQ5NUM5NEB0ZWNoYWNjdC5hZG9iZS5jb20iLCJ0eXBlIjoiYWNjZXNzX3Rva2VuIiwiYXMiOiJpb XMtbmExIiwiZmciOiJTRDRZQUNZSEhQSDdPRkFBQUFBQUFFFST09PT09PSIsIm1vaSI6IjM3NTliZjQxIwiYiI dTVTd3b1hIZkZSQk5xQmo3M2Z4anc9PSIsImV4cGlyZXNfaW4iOiI4NjQwMDAwMCIInNjb3BlIjoib3Blbmlk LEFkb2JlSUQsdGFyZ2V0X3NkayxyZWFkX29yZ2FuaXphdGlvbnMsYWRkaXRpb25hbF9pbmZvLnP2pl3RlZl FByb2R1Y3RDb250ZXh0IiwiY3JlYXRlZF9hdCI6IjE1MTY3NDgxNjU5MzYifQ.fVJVREKZH3PM71-Y05Kqqqkqqqqkqqqqq q_O_z7BL5NL6S4ypNoSwLuqR9WOiXsF0GYcWZr6oO-jgYj8WrRePQLkg4GSoVthSbbXU6aqajPV2TsFNHXRu FBhql0e2eVCEE_pVI9O_uCa8RloGjJuFyyVoQFEIJzC7Q-OAnkXMT7xD-3r1cEV2xP_N3s86t34M5udO4fjas RCJtAS1BEZOotlF_rB0kfvCZR9Krf-SVi_VedpsK7ipoJGfs7CLdN-_a4YGTC2CBJXwdK-4T0QJRkWedr8ooS0tzzfVc4WEFZ w1edi-OYSuIbXf-Obl5R9NCzi5RMceiGTyGMyRrEcmy3WQ&quot;` |
    | — |


- <b>Exemple d’appel API :</b>

   - [!DNL Target]
   - Exécuter [!DNL Target] appel avec jeton d’accès

![rtaimage_18_](https://helpx.adobe.com/content/dam/help/en/marketing-cloud-core/kb/adobe-io-authentication-step-by-step/jcr%3acontent/main-pars/procedure/proc_par/step_4/step_par/image/rtaimage_18_.png "rtaimage_18_")
