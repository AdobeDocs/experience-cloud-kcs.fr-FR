...
<b>Environnement</b><br><br>Experience Manager as a Cloud Service<br><br> <br><br><b>Problème/Symptômes</b><br><br>Dans la version 2023.02.11382, quelques améliorations ont été apportées au <b>GraphQL</b> (Mise en oeuvre du langage de requête Open Source) qui peut entraîner un comportement inattendu dans le code de l’application, au cas où il s’appuyait sur le comportement erroné des anciennes versions. 
<br>La résolution ci-dessous permet d’obtenir que Adobe Experience Manager (AEM) GraphQL renvoie désormais du contenu avec *null* valeurs uniformément sur toutes les opérations de filtre. <br>

## Résolution {#resolution}

`CONTAINS_NOT` et *null* values<br> <br>La première modification affecte les conditions de filtrage en utilisant la variable `CONTAINS_NOT` de l’opérateur. Dans les versions précédentes, cela `CONTAINS_NOT` ne renvoyait pas les fragments de contenu contenant *null* dans le champ auquel la condition de filtrage a été appliquée. Cela est incompatible avec des opérateurs similaires tels que `EQUALS_NOT` ou `NOT_AT`.<br>Assurez-vous que votre application cliente est en mesure de gérer *null* valeurs renvoyées par AEM lors de l’utilisation de `CONTAINS_NOT`.<br> <br><b>Exemple:</b>
...
<br>Appliquer les modes `ALL_OR_EMPTY/ALL` et *null* values<br>
...
<br>Appliquer les modes : `INSTANCES`, `_instances: 0` et *null* values<br>
...
<br>Améliorations susceptibles d’affecter les clients `ft-sites-97` enabled<br>
...
<br>gestion nulle avec des conditions qui vérifient la non-égalité<br>
...
<br>Opérateur de filtre `NOT_AT` avec des valeurs de date/heure<br>
...
<br>Tableaux : `INSTANCES` avec `_instances: 0` ne fonctionne pas correctement<br>
