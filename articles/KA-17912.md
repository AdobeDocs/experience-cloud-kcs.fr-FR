...
Ce document explique comment [!DNL dispatcher] mise en cache se produit et comment elle peut être configurée<br><br>
...
Lorsque chaque requête traverse la variable [!DNL dispatcher] les demandes suivent les règles configurées pour conserver une version mise en cache localement en réponse aux éléments éligibles
...
[!DNL Dispatcher] contrôle ce qui est qualifié de mise en cache dans la variable `/cache {` de tout fichier de ferme. 
...
C&#39;est l&#39;un de ces atouts d&#39;une fonctionnalité de [!DNL dispatcher]. Si l’éditeur est en cours de chargement ou ne répond plus, il génère généralement un code de réponse http 502 ou 503. Si cela se produit et que cette fonction est activée, la variable [!DNL dispatcher] sera chargé de toujours servir le contenu qui se trouve toujours dans le cache comme un effort optimal, même s’il ne s’agit pas d’une nouvelle copie. Il est préférable de diffuser quelque chose si vous l’avez plutôt que de simplement afficher un message d’erreur qui n’offre aucune fonctionnalité.
...
L’un des comportements normaux de la variable [!DNL Dispatcher] est que si une requête comporte un paramètre de requête dans l’URI (généralement affiché comme `/content/page.html?myquery=value`) ignore la mise en cache du fichier et accède directement à l’instance AEM. Cette requête est considérée comme une page dynamique et ne doit pas être mise en cache. Cela peut avoir des effets sur l’efficacité du cache.

Si des pages dans AEM prennent `GET` les arguments/paramètres de requête dans la ligne d’adresse qui aident la page à fonctionner, mais ne rendent pas de code html différent sont de bons candidats pour cet élément de configuration. Vous pouvez le dire. [!DNL dispatcher] les arguments à ignorer et à mettre en cache la page.
...
Maintenant, lorsque la variable [!DNL dispatcher] voit la requête ; elle ignore le fait que la requête a le paramètre de requête ?reference et met toujours la page en cache.



<b>Mise en cache des en-têtes de réponse</b>

Il est assez évident que la variable [!DNL dispatcher] caches `.html` pages et clientlibs, mais saviez-vous qu’il peut également mettre en cache des en-têtes de réponse spécifiques le long du contenu dans un fichier portant le même nom, mais avec un `.h` extension de fichier. Cela permet à la prochaine réponse non seulement au contenu, mais aussi aux en-têtes de réponse qui doivent l’accompagner depuis le cache.
...
Ces valeurs, lorsqu’elles sont mises en cache, sont supprimées par défaut et la variable [!DNL apache] httpd webserver effectue sa propre tâche de traitement de la ressource avec ses méthodes de gestion de fichiers normales, qui se limitent normalement à des suppositions de type mime en fonction des extensions de fichier.

Si vous disposez de la variable [!DNL dispatcher] mettre en cache la ressource et les en-têtes souhaités, vous pouvez exposer l’expérience appropriée et vous assurer que tous les détails la transmettent au navigateur des clients.
...
Gardez à l’esprit que AEM doit toujours être configuré pour envoyer des en-têtes TTL pour [!DNL dispatcher] pour les honorer. Activer/désactiver cette fonction active uniquement la variable [!DNL dispatcher] pour savoir quand supprimer les fichiers pour lesquels AEM a envoyé des en-têtes de contrôle du cache. Si AEM ne commence pas à envoyer des en-têtes TTL alors [!DNL dispatcher] Je ne ferai rien de spécial ici.
