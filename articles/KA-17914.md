...
Le document décrit les entrées de journal courantes que vous verrez, ce qu’elles signifient et comment les traiter.<br><br>[Table des matières](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-17490.html?lang=fr)
...
Depuis [!DNL dispatcher] module 4.2.x, ils ont commencé à décourager les utilisateurs d’utiliser le type de correspondance suivant dans leur fichier de filtres :
...
Comprenez que la variable [!DNL dispatcher]Les règles de ont été définies pour filtrer cette requête. Dans ce cas, la page qui a tenté d’être visitée a été refusée intentionnellement et nous ne voudrions rien faire avec cela.
...
Cela se produit lorsque l’adresse IP incorrecte est configurée dans la section renders de votre ferme de serveurs. Cette instance ou l’instance AEM a cessé de répondre ou d’écouter et le [!DNL dispatcher] Je ne peux pas l&#39;atteindre.
...
Cela signifie que l’instance AEM avait un socket ouvert qu’elle pouvait atteindre et avait expiré avec la réponse. Cela signifie que votre instance d’AEM était trop lente ou incorrecte et [!DNL dispatcher] a atteint les paramètres de délai d’expiration configurés dans la section de rendu de la ferme de serveurs. Augmentez le paramètre de délai d’expiration ou assurez-vous que votre instance AEM est saine.
...
Cette erreur se produit lorsque vous avez configuré votre [!DNL dispatcher] pour utiliser le filtre automatique dynamique, autorisez les URL de redirection vers un microsite, mais n’avez pas terminé la configuration en installant le package sur le moteur de rendu AEM.
